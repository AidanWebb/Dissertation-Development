...
def chat_page(username):
   ...
   public_key = user_db.fetch_public_key(username)
   ...
   for friend_username in user.get('friends', []):
       friend_public_key = user_db.fetch_public_key(friend_username)
       if friend_public_key:
           contacts_public_keys[friend_username] = friend_public_key

   return render_template('chat_page.html', user=user, private_key=private_key, public_key=public_key,
